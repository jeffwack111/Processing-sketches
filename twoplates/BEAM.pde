class BEAM{
  ...
  void show(int x, int y){
    ...
    for(int n=0;n<120;n++){
      ...
      CXN eks = mul(X,pha);
      CXN why = mul(Y,pha);
      point(x+eks.re()*125,y-why.re()*125);
    }
  }
}

BEAM pass(BEAM beam, float phi, float theta){
  CXN oneone = new CXN(cos(phi), sin(phi)*cos(2*theta));
  CXN onetwo = new CXN(0, sin(phi)*sin(2*theta));
  CXN twoone = new CXN(0, sin(phi)*sin(2*theta));
  CXN twotwo = new CXN(cos(phi), -sin(phi)*cos(2*theta));
